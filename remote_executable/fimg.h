...
const std::string target_out_name("empty_bottle");
...
Double_t tof_cut_low = 7057.0;
Double_t tof_cut_up = 66477.0;
...
Double_t n_CFib_1cm = (1 /*cm*/) * (1.8 /*g/cm3*/) * (6.02214076e23 /*atoms/mole*/) * (1e-24 /*cm2/barn*/) / (12.011 /*g/mole*/);

Double_t n_Ar_bottle = (11 /*cm*/) * ((ar_bottle_pressure)/(8.31446261815324 * ar_bottle_temp * 1e6)) * (6.02214076e23 /*atoms/mole*/) * (1e-24 /*cm2/barn*/);
...
//Argon Bottle
std::vector<Int_t> f_out_t_arBot_ts = {117665, 117680, 117684, 117685, 117686, 117687, 117688, 117692, 117693, 117710, 117711, 117712, 117713, 117714, 117716, 117717, 117718};

Double_t cutFunction(Double_t x){
    // A * (log(x))^2 + B + log(x) + C
    Double_t A = ;
    Double_t B = ;
    Double_t C = ;
...
Double_t EnergyToTOF(Double_t e){ //e is in eV
    ...
    Double_t TOF = (flight_path_length_FIMG/speed_of_light) * correction_factor;
    return TOF; //tof in seconds
}
    
Double_t TOFToEnergy(Double_t t){ //t is in seconds
    Double_t denom_term = (flight_path_length_FIMG)/(speed_of_light * t);
...
Double_t TOFToEnergy(Double_t t, Double_t rf_length){ //t is in seconds, rf_length is in m
    Double_t denom_term = (flight_path_length_FIMG + rf_length)/(speed_of_light * t);
    Double_t denominator = 1.0 - (denom_term * denom_term);
    Double_t factor = std::sqrt(1.0 / denominator) - 1.0;
    Double_t energy = neutron_mass * factor;
    return energy * 1e6;  //e in eV
